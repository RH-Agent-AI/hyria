...
prompt_analyse.input_variables = [ "cv_text","job_description","hr_conversation","agent_scratchpad"]
...
prompt = ChatPromptTemplate.from_messages([
    ...
    (MessagesPlaceholder(variable_name="chat_history")),
    ("human", "Get the resume in {cv} using the tool"),
    (MessagesPlaceholder(variable_name="agent_scratchpad")),
    ("placeholder", "{agent_scratchpad}"),
])
...
agent2 = create_tool_calling_agent(
    llm=llm, tools=tools, prompt=prompt_analyse
    )
...
def execute_agent_analyse(job_description, cv, hr_conversation):
    out = agent_executor.invoke({
